...
## Web Server Pipeline Improvements (2026-05-10)

### Three-call LLM pipeline

The web server now uses a three-stage pipeline per conversation turn instead of
...
User message
    ...
    ▼
[1] Extractor (Haiku)
    Input: preceding bot question + user message
    Output: structured Extraction — symptoms, OLDCARTS fields,
            engagement signal, is_off_topic, is_correction
    │
    ▼
[2] Planner (Haiku)
    Input: phase, chief complaint, turn summaries, filled OLDCARTS,
           last bot question, checklist state
    Output: PlannerDecision — what to acknowledge, OLDCARTS topics
            already answered in summaries, dimensions to skip,
            non-OLDCARTS topics already covered (do_not_ask_again)
    │
    ▼
[3] Renderer (Sonnet)
    Input: clinical context built from session + PlannerDecision
    Output: natural prose response
```

**Why:** The original single-call approach caused repeated questions (the renderer
had no structured view of what had already been covered), off-topic misfires (a
hard `has_signal` structural check was too narrow and rejected legitimate clinical
responses), and safety filter false positives (ingredient whitelist ran with empty
permitted list, blocking words like "heart" and "muscle").

**Key files:**
- `crates/web-server/src/extract.rs` — Extractor, now receives preceding question for context
- `crates/web-server/src/planner.rs` — Planner, new file
- `crates/intake-agent/src/context.rs` — `build_context_v2` accepts `PlannerHints`

### Safety filter fixes

Two fixes to `crates/intake-agent/src/safety.rs`:
1. Ingredient whitelist check now skips entirely when no candidates exist (permitted
   list is empty). Previously it ran and blocked normal clinical language like
   "cardiovascular support" and "muscle tension" that happen to appear in the 70k-entry
   registry.
2. Unigram minimum length raised from 5 → 9 characters. Short words (heart, sleep,
   iron, ginger) are normal clinical English; true supplement unigrams that need
   blocking (quercetin, magnesium, ashwagandha) are longer.

### Citation ranking improvement

`gather_citations_for_ingredients` in `crates/web-server/src/handler.rs` now uses
tiered symptom-specific scoring rather than keyword matching against the response text:

| Tier | Points | Condition |
|------|--------|-----------|
| 1 | 1000 | `target_node` directly matches a user symptom |
| 2 | 100× | Citation sentence contains a symptom term |
| 3 | 10 | `target_node` matches a body system |
| 4 | 1× | Sentence contains a system term |
| 5 | + | Confidence score tiebreaker |

The session now separately tracks `symptom_terms` (raw complaint + mapped symptoms +
active profile IDs) and `system_terms` (mapped systems) for ranking use.

## Graph Coverage Gap and Fix (Phase 9 — in progress 2026-05-10)
...
Many symptom nodes that exist in the graph have zero outgoing edges — no
`presents_in` connection to any body system. The executor queries them and returns
0 candidates. The renderer then free-wheels: it produces plausible-sounding
supplement text based on LLM training data alone, with no graph backing and no
citations.

**Confirmed gaps (2026-05-10):**
- Headache: exists as `symptom` entity, 10 incoming `has_adverse_reaction` edges,
  0 outgoing edges — no `presents_in` path to any system
- Pain: likely the same situation (not yet audited)

**How it was discovered:** Executor consistently logged `0 candidate(s)` even for
fully completed intake interviews. Graph API confirmed Headache had no outgoing edges.

**Why the gap exists:** `presents_in` edges are generated by the NSAI extraction
pipeline from research literature, not manually seeded. NSAI encountered Headache
primarily as an adverse reaction target and never generated `presents_in` edges for
it. The intake config correctly maps "headache" → nervous system, but that mapping
lives in Postgres (intake symptom profiles), not in the graph edge table.

### The fix

Three steps, tracked in `supplementology/docs/PLAN.md` Phase 6:

1. **UMLS symptom import** — download T184 "Sign or Symptom" concepts for all
   wellness-relevant domains, upsert as `symptom` entity nodes, load English synonyms
   into the merge store. CLI: `supplementology import umls-symptoms`

2. **NSAI edge generation** — run NSAI over symptom nodes that have no `presents_in`
   edges. NSAI queries the literature and generates the missing edges.

3. **Coverage audit** — verify all 17 intake symptom profiles have a working path:
   symptom node exists → has `presents_in` edge → target system has `acts_on` edges
   from ingredients.

### Done criteria

- All 17 intake symptom profiles return ≥1 candidate from the graph executor
- Citation panel appears during recommendation for all primary complaint categories
- No more "0 candidate(s)" log lines for supported symptoms
...
### Implementation status — COMPLETE ✓ (2026-05-04)

1. ✓ Fields added to SurrealDB schema (`confirmation_status`, `citation_pmids`, `confirmation_source`)
2. ✓ Confirmation pass built (`supplementbot confirm-edges`) using `/v1/graph-feed` API
3. ✓ Query engine confidence scoring updated with confirmation multiplier
4. ✓ Confirmation pass run for all 19 original ingredients (Phase 8a)
5. ✓ NSAI loop run for 77 new ingredients across 8 batches (Phase 8b)
6. ✓ Final confirm-edges run — 159 total edges matched, 151 new citations stored

**Graph as of 2026-05-04:** 213 nodes, 1,794 edges (up from 129 nodes, 991 edges)

**Next:** supplementology Phase 5c (PubMed Trials importer) will dramatically improve
citation coverage for amino acids and performance compounds that currently have 0 confirmed edges.
